...
interface MultiSelectWithDescriptionProps {
  ...
  searchInDescription?: boolean; // ค้นหาใน description ด้วยไหม
}

export function MultiSelectWithDescription({
  options,
  value,
  onChange,
...
}: MultiSelectWithDescriptionProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');
  const containerRef = useRef<HTMLDivElement>(null);
  const inputRef = useRef<HTMLInputElement>(null);

  // Filter options (ค้นหาใน label และ description)
          ...
          selectedOptions.length <= maxDisplayItems ? (
            selectedOptions.map((option) => (
              <span
                ...
                title={option.description} // แสดง description เป็น tooltip
              >
                {option.label}
                {!disabled && (
                  <button
        ...
        <div className="bg-popover absolute top-full z-50 mt-1 w-full rounded-md border shadow-lg">
          ...
          <div className="p-2">
            <input
              ref={inputRef}
              type="text"
              placeholder={`ค้นหา${searchInDescription ? ' (ค้นหาใน description ได้)' : ''}...`}
              value={searchTerm}
              ...
              filteredOptions.map((option) => {
                ...
                return (
                  <div
                    key={option.value}
                    className={cn(
                      'hover:bg-accent hover:text-accent-foreground flex cursor-pointer items-start justify-between px-3 py-3 text-sm',
                  ...
                  >
                    ...
                    {isSelected && (
                      <Check className="text-primary mt-0.5 ml-2 h-4 w-4 flex-shrink-0" />
                    )}
                  </div>
                );
              })
